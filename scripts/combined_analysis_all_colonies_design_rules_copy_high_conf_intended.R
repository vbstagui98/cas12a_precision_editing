...
## alternate run: intended edits must also pass QUAL/AF/DP high-confidence filters
library(dplyr)
library(ggplot2)
library(Biostrings)
library(tidyverse)
...
output_dir = file.path(project_root, "results", "07_genome_wide_design_rules_high_conf_intended")
...
high_conf_variant_calls_all = normalized_vcf_all %>%
  mutate(
    QUAL = as.numeric(QUAL),
    DP = as.numeric(DP),
    AF = as.numeric(frc_alt)
  ) %>%
  filter(QUAL >= 20, AF >= 50, DP >= 4)

designs_all = designs_all %>%
  mutate(
    edited = ifelse(sample_vars %in% high_conf_variant_calls_all$sample_vars, 1, 0),
...
background_variants = variant_calls_all %>%
  ...
  annotate_nearest_score_sv("CHR", "POS", score_sv_sites) %>%
  mutate(
    high_conf_intended = is_intended_variant & QUAL >= 20 & AF >= 50 & DP >= 4,
...
high_conf_intended_variants = background_variants %>% filter(high_conf_intended)
high_conf_non_target_variants = background_variants %>% filter(high_conf_non_target)

intended_variant_confidence_summary = intended_sites %>%
  left_join(
    background_variants %>%
      filter(is_intended_variant) %>%
      group_by(sample) %>%
      summarize(
        intended_variant_detected = n() > 0,
        high_conf_intended_detected = any(high_conf_intended),
        max_intended_qual = max(QUAL, na.rm = TRUE),
        max_intended_af = max(AF, na.rm = TRUE),
        max_intended_dp = max(DP, na.rm = TRUE),
        .groups = "drop"
      ),
    by = "sample"
  ) %>%
  mutate(
    intended_variant_detected = coalesce(intended_variant_detected, FALSE),
    high_conf_intended_detected = coalesce(high_conf_intended_detected, FALSE)
  )
...
write.csv(high_conf_intended_variants, file.path(tables_dir, "high_conf_intended_variants.csv"), row.names = FALSE)
write.csv(high_conf_non_target_variants, file.path(tables_dir, "high_conf_non_target_variants.csv"), row.names = FALSE)
write.csv(intended_variant_confidence_summary, file.path(tables_dir, "intended_variant_confidence_summary.csv"), row.names = FALSE)
...
print(tibble(
  metric = c(
    ...
    "High-confidence intended variants",
  ...
  value = c(
    ...
    DR_seq,
    nrow(high_conf_intended_variants),
